...
// ── Ficha: componente estable (memo) para evitar pérdida de foco ─────────────
// Extraído fuera de CriteriosTab y envuelto en React.memo para que React NO
// desmonte/remonte los inputs en cada keystroke. La causa original del bug era
// que Ficha se definía como función inline dentro del render del padre →
// React la veía como un componente nuevo cada vez → desmontaba los <textarea>.
const Ficha = memo(function Ficha({
  c,
  onUpdate,
}: {
  c: Criterio;
  onUpdate: (id: string, cambios: Partial<Criterio>) => void;
}) {
  return (
      ...
      <div className="crit-campos">
        ...
        <label className="filter-group">
          ...
          <select className="input" value={c.accion}
            onChange={(e) => onUpdate(c.id, { accion: e.target.value as Accion })}>
            <option value="">— sin definir —</option>
        ...
        <label className="filter-group">
          ...
          <textarea className="input crit-textarea" value={c.notas} rows={2}
            ...
            onChange={(e) => onUpdate(c.id, { notas: e.target.value })} />
        </label>
      </div>
      <div className="crit-estado-row">
        ...
        {(['pendiente', 'vigente', 'confirmado'] as Estado[]).map((e) => (
          <button key={e} className={`btn${c.estado === e ? ' crit-estado-activo' : ''}`}
            onClick={() => onUpdate(c.id, { estado: e })}>
...
export function CriteriosTab() {
  ...
  // Persistir en localStorage cada vez que cambian los criterios.
  // Se usa un ref para saltar la primera ejecución (carga inicial).
  ...
  // useCallback estabiliza la referencia para que Ficha (memo) no se
  // re-renderice innecesariamente y los inputs mantengan el foco.
  const actualizar = useCallback((id: string, cambios: Partial<Criterio>) => {
    setCriterios((prev) =>
